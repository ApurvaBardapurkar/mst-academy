  ...
  return (
    ...
    >
      ...
      >
        ...
        <div
          className="mt-5 h-1.5 overflow-hidden rounded-full bg-[var(--border)]"
          aria-hidden
        >
          <div
            ...
            style={{
              width: active ? "100%" : "35%",
              background: `linear-gradient(90deg, ${color}, rgba(227,30,36,0.2))`,
        ...
        <div className="flex items-center justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <h4 className="mt-0.5 truncate text-sm font-extrabold text-[var(--text)]">
                {title}
              </h4>
            </div>
          </div>
      ...
      >
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="mt-1 line-clamp-2 text-xs font-bold text-[var(--text)]">{title}</p>
          </div>
          <div className="shrink-0">
            {locked ? (
              <Lock className="h-4 w-4 text-[var(--text-muted)]/70" />
...
export function LearningRoadmap({ curriculum }: { curriculum: Curriculum }) {
  ...
  const nodesAndEdges = useMemo(() => {
    ...
    const moduleGapY = isMobile ? 200 : 210;
    const subGapY = isMobile ? 130 : 140;
    ...
    // ── Module view: ONLY modules in selected phase (single column, no overlap) ──
    if (activePhaseId) {
      const style = PHASE_STYLE[activePhaseId as keyof typeof PHASE_STYLE];
      const color = style?.color ?? "#e31e24";
      const modules = modulesInActivePhase;
      const startX = isMobile ? 20 : isTablet ? 60 : 120;
      ...
      modules.forEach((mod, i) => {
        ...
        nodes.push({
          id: `module-${mod.id}`,
          type: "moduleCard",
          position: { x: startX, y: i * moduleGapY },
          data: {
            module: mod,
            phaseId: activePhaseId,
            color,
        ...
        if (i > 0) {
          edges.push({
            id: `e-mod-${modules[i - 1]!.id}-${mod.id}`,
            source: `module-${modules[i - 1]!.id}`,
    ...
    // ── Phase view: 4 phases in responsive grid ──
    const phaseGapX = isMobile ? cardW + 24 : isTablet ? 340 : 400;
    const phaseGapY = isMobile ? 220 : 0;
    ...
    if (!isMobile) {
      for (let i = 0; i < phaseOrder.length - 1; i++) {
        edges.push({
          id: `e-phase-${i}`,
          source: `phase-${phaseOrder[i]}`,
          target: `phase-${phaseOrder[i + 1]}`,
          type: "smoothstep",
          animated: true,
          style: {
            stroke: isLight ? "rgba(59,130,246,0.35)" : "rgba(168,85,247,0.45)",
            strokeWidth: 2.5,
          },
          markerEnd: {
  ...
  const graphHeight = useMemo(() => {
    if (activeModuleId && activeModule) {
      return Math.max(480, 220 + activeModule.submodules.length * (isMobile ? 130 : 140));
    }
    if (activePhaseId) {
      const count = modulesInActivePhase.length;
      return Math.max(480, count * (isMobile ? 200 : 210) + 80);
    }
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 mx-auto mt-4 max-w-7xl px-4 pb-8 sm:px-6">
        <div
          className="roadmap-graph-shell relative overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--surface)]/30 backdrop-blur-md"
          style={{
            height: isMobile
        ...
        >
          ...
          >
            ...
            {!isMobile && (
              <MiniMap
                nodeColor={() => "#e31e24"}
                maskColor={isLight ? "rgba(0,0,0,0.18)" : "rgba(0,0,0,0.62)"}
                className="!border !border-[var(--border)] !bg-[var(--surface)]/60"
              />
            )}
